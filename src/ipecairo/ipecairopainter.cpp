...
static void drawImage(cairo_t *cr, const PdfDict *d, const PdfResourceBase *r, double opacity)
{
  ...
  cairo_surface_t *image =
    cairo_image_surface_create_for_data((uint8_t *) pixels.data(),
					CAIRO_FORMAT_ARGB32,
					width, height,
					4 * width);
  cairo_save(cr);
...
CairoPainter::CairoPainter(const Cascade *sheet, Fonts *fonts,
  ...
  : Painter(sheet), iFonts(fonts), iCairo(cc), iZoom(zoom), iPretty(pretty)
{
  iDimmed = false;
}
...
void CairoPainter::createPattern()
{
  ...
  double patternType, paintType, xstep, ystep;
  if (pat && pat->getNumber("PatternType", patternType, nullptr)
      && pat->getNumber("PaintType", paintType, nullptr)
      && pat->getNumber("XStep", xstep, nullptr)
      && pat->getNumber("YStep", ystep, nullptr)) {
    ipeDebug("Fill pattern /%s PatternType %g PaintType %g xstep %g ystep %g", ps.iFillPattern.z(),
	     patternType, paintType, xstep, ystep);
    ...
    if (pat->get("Matrix", nullptr) == nullptr) {
	...
	CairoPainter painter(iCascade, iFonts, cc, 1.0, false);
