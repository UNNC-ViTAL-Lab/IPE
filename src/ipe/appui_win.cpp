...
static HBITMAP loadIcon(String action, int w, int h, int r0, int g0, int b0)
{
  ...
  Gdiplus::Bitmap *bitmap = Gdiplus::Bitmap::FromFile(WString(fname).data(), FALSE);
...
#define MODE_BUTTONS 24
  ...
  hLayers = CreateWindowExW(0, WC_LISTVIEWW, nullptr,
			    ...
			    (HMENU) IDC_LAYERS,
			    hInst, nullptr);
  (void) ListView_SetExtendedListViewStyle(hLayers, LVS_EX_CHECKBOXES);
  ...
  if (iLeftDockFloats)
    // SendMessage(hEditTools, TB_SETSTATE, actionId("dock_visible"),
    // (LPARAM) (TBSTATE_ENABLED | TBSTATE_CHECKED));
...
void AppUi::layoutChildren(bool resizeRebar)
{
  ...
  if (visLeftDock) {
    ...
    MoveWindow(iPathView->windowId(), uiscale(16),
	       tbh + uiscale(24 + dy * 2 + 28 + iUiGap / 2),
...
static const char * const aboutText =
  ...
  " * Hee-Kap Ahn\n"
  " * Martin Ziegler\n\n"
  "If you enjoy Ipe, you can become a member of the exclusive community of "
