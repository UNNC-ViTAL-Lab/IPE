...
- (NSTouchBar *) makeTouchBar
{
  ...
  bar.customizationAllowedItemIdentifiers =
	...
	@"snapvtx", @"snapctl", @"snapbd", @"snapint", @"snapgrid", @"snapangle", @"snapauto" ];
	...
	makeItemForIdentifier: (NSTouchBarItemIdentifier) identifier
{
  ...
  for (size_t tag = 0; tag < (sizeof(touchbar_action)/sizeof(const char *)); ++tag) {
    if (idf == touchbar_action[tag]) {
      ...
      NSButton *b;
      if (title == nullptr) {
	auto image = loadIcon(touchbar_action[tag]);
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    if (iDelegate.touchBar != nil) {
      auto bar = (NSTouchBar *) iDelegate.touchBar;
