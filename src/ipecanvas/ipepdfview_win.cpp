				  ...
				  LPARAM lParam)
{
  ...
  case WM_SIZE:
    if (view) {
      view->updateSize();
      PostMessage(view->target, 0x8000, 0, 0);
    }
    break;
  case WM_LBUTTONDOWN:
    if (view)
      PostMessage(view->target, 0x8000, 0, 1);
    break;
  case WM_RBUTTONDOWN:
  case WM_MBUTTONDOWN:
  case WM_XBUTTONDOWN:
    if (view)
      PostMessage(view->target, 0x8000, 0, 2);
    break;
  case WM_CLOSE:
...
PdfView::PdfView(HWND parent, HWND target, HINSTANCE hInstance) : target(target)
{
