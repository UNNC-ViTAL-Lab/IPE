...
private:
  ...
  void timerElapsed();
  void handlePdfViewMessage(int param);
...
void AppUi::initUi()
{
  ...
  */
  ...
  HMENU hSubMenu = CreatePopupMenu();
  AppendMenuA(hSubMenu, MF_STRING, EShowPresentation, "Show &Presentation");
  ...
  AppendMenuA(hSubMenu, MF_STRING, ESetTime, "&Set time");
  ...
  iCurrent = new PdfView(hwnd, hwnd);
  iNext = new PdfView(hwnd, hwnd);
  iScreen = new PdfView(nullptr, hwnd, hInstance);
...
void AppUi::handlePdfViewMessage(int param)
{
...
void AppUi::setPdf()
{
  iScreen->setPdf(iPdf.get());
  iCurrent->setPdf(iPdf.get());
  iNext->setPdf(iPdf.get());
}
...
static const char * const aboutText =
  ...
  "IpePresenter is now released as part of Ipe under the GNU Public License.\n"
  ...
  " * Hee-Kap Ahn\n"
  " * Martin Ziegler\n\n"
  "If you enjoy IpePresenter, you can become a member of the exclusive community of "
...
void AppUi::aboutIpePresenter()
{
  ...
  MessageBoxA(hwnd, &buf[0], "About IpePresenter",
				...
				LPARAM lParam)
{
  ...
  case PdfView::WM_PDFVIEW:
    if (ui)
      ui->handlePdfViewMessage(lParam);
    break;
  default:
    break;
  }
